...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        val cachedKeywords = prefs.getString("home_keywords", null)?.split("|")?.filter { it.isNotBlank() }
        ...
        // Fortune식 입장 애니(세션 1회)
        if (!homeAnimPlayed) { animateHomeCardLikeFortune(aiReportCard); homeAnimPlayed = true }
        else aiReportCard.visibility = View.VISIBLE
        ...
        if (currentUid == null) {
            ...
            FirestoreManager.countDreamEntriesForWeek(currentUid, thisWeekKey) { count ->
                ...
                if (count < 2) {
                    ...
                    FirestoreManager.loadWeeklyReportFull(currentUid, thisWeekKey) { f, k, a, emoL, emoD, thL, thD, sourceCount, rebuiltAt, tier, proAt, stale ->
                        ...
                        if (needRebuild) {
                            FirestoreManager.aggregateDreamsForWeek(currentUid, thisWeekKey) {
                                ...
                                FirestoreManager.loadWeeklyReport(currentUid, thisWeekKey) { ff, kk, aa, sc2 ->
                                    ...
                                    aiReportTitle.post { if (isAlive()) applyHomeSummary(aiReportTitle, aiReportSummary, btnAiReport, ff, kk, aa, sc2) }
                                }
                            }
                        } else {
                            FirestoreManager.loadWeeklyReport(currentUid, thisWeekKey) { ff, kk, aa, sc2 ->
        ...
        aiReportCard.setOnClickListener {
            it.startAnimation(AnimationUtils.loadAnimation(requireContext(), R.anim.fade_in_scale_95))
            openAIReport()
        }
    ...
    private fun openAIReport() {
        ...
        if (userId == null) {
            if (!lastFeeling.isNullOrBlank()) {
                ...
                context?.let { Toast.makeText(it, "이번 주 꿈을 2개 이상 기록하면 AI 리포트를 볼 수 있어요.", Toast.LENGTH_SHORT).show() }
            }
            return
        }
        navigateTo(AIReportFragment().apply {
