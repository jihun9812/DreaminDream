...
class AIReportFragment : Fragment() {
    ...
    /** ✅ 구독 제거 버전: 프리미엄 텍스트/분기 없이 단순 CTA 상태만 */
            ...
            FirestoreManager.loadWeeklyReportFull(
                ...
                if (hasBasic && hasDist && (stale || sourceCount != dreamCount)) {
                    FirestoreManager.aggregateDreamsForWeek(userId, weekKey, requireContext()) { ok ->
                        ...
                        if (ok) {
                            isReloading = false
                            reloadForWeekInternal(weekKey) // 최신 감정/키워드/분포 재로딩
                        } else {
                            // 실패 시: 이전 값 유지 + 헤더에 stale로 표시
        ...
        btnWatch.setOnClickListener {
            ...
            AdManager.showRewarded(
                ...
                onRewardEarned = {
                    ...
                    adEarned = true
                    textStatus.text = getString(R.string.reward_confirmed); bs.dismiss()
                ...
                onClosed = {
                    ...
                    Snackbar.make(reportCard, getString(R.string.ad_closed_no_reward), Snackbar.LENGTH_SHORT).show()
                    applyProButtonState()
                },
                onFailed = { reason ->
                    if (!isUiAlive()) return@showRewarded
